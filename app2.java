...
String SetPin(int oldPin , int newPin){
  String r="1";
  String getpin= "SELECT * FROM Customer C WHERE C.PIN= " + oldPin;
  try (Statement checkpin= _connection.createStatement()){
      try (ResultSet resultSet = checkpin.executeQuery(getpin)){
        if(resultSet.next()){

          String updatepin= "UPDATE CUSTOMER SET PIN = ? WHERE  PIN = ?  ";
          try(PreparedStatement update= _connection.prepareStatement(updatepin)){
            update.setInt(1, oldPin);
            update.setInt(2, newPin);
